...
contract('MasterChef', ([alice, bob, carol, referrer, treasury, dev, fee, owner]) => {
    beforeEach(async () => {
        ...
        this.referral = await OldSchoolReferral.new({ from: owner });
        this.chef = await MasterChef.new(this.neon.address, '100', '1000', { from: owner });

        await this.neon.transferOwnership(this.chef.address, { from: owner });
        await this.referral.updateOperator(this.chef.address, true, { from: owner });
        await this.chef.setOldSchoolReferral(this.referral.address, { from: owner });
